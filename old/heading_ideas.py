...
class Heading():
    ...
    def __init__(self, batch_size, num_targets):
    ...
    def update_sentence_from_vector(self,idx):
        ...
        for i in range(self.num_targets):
            ...
            string += f"The {enumerations[i]} {targets[0]} {insides[0]} the {dir_1} {dir_2} {positions[0]} {spaces[0]}."
    ...
    def update_sentence_vectors(self,idx):
        ...
        for i in range(self.num_targets):
            ...
            val_tensor = np.clip(np.array([direction_map[d1] for d1 in dir_types_1]) + np.array([direction_map[d2] for d2 in valid_d_types_2]),a_min=0.0, a_max=1.0)
            uncertainty = (np.random.rand(1, 2) * 0.15) * - val_tensor
            ...
            self.heading_pos_grid[idx, i*2 + 0] = (val_tensor[:, 0] * self.grid_width)
            self.heading_pos_grid[idx, i*2 + 1] = (val_tensor[:, 1] * self.grid_height)
    ...
    def _initalize_heading(self,eval, model):
        ...
        self.heading_pos_grid = torch.zeros((self.batch_size, 2 * self.num_targets), device=self.device)
        ...
        for i in range(self.batch_size):
            self.update_sentence_vectors(i)
            self.update_sentence_from_vector(i)
        ...
        return {
            ...
            "goal": self.heading_pos_grid.cpu().numpy()
        }
