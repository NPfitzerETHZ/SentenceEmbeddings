...
def describe_image(
    ...
    *patch_centroid* – (row, col) floats in grid coordinates.
    ...
    if include_number:
        ...
        tgt_snip = f"{num_targets} target{plural}"
    else:
        tgt_snip = "the targets"

    opening = "You are leading a team of autonomous robots tasked with finding " + tgt_snip + "."
...
def target_and_color():
    output_file = "sentences/gemini_patch_dataset_target_scale_position.jsonl"
    json_output_file = "sentences/gemini_patch_dataset_target_scale_position.json"
    start_index = 0
    buffer = []
