...
from process_gemini_data import collect_train_test_data_grid, collect_train_test_data_from_embeddings_attributes
...
json_data_file = "data/language_data_complete_single_target_color.json"
...
class MultiHeadDecoder(nn.Module):
    def __init__(self, emb_size, goal_out_size, num_classes, shared_depth=0, hidden_size=256):
...
def combined_loss_fn(model, emb, goal, class_goal, alpha=0.01, beta=1.0):
    goal_pred, class_logits = model(emb)
...
for llm, llm_name in llms.items():
    ...
    train, test = collect_train_test_data_from_embeddings_attributes(json_path=json_data_file,train_ratio=0.8,test_ratio=0.2, device="mps")

    device = torch.device("cuda" if torch.cuda.is_available() else "mps")
    model = MultiHeadDecoder(train["task_embedding"].shape[1],train["goal"].shape[1],num_classes=11).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i in range(train["task_embedding"].shape[0] // batch_size):
            ...
            # Compute losses
            loss, goal_loss, class_loss = combined_loss_fn(model, emb, goal, class_goal_onehot)
        ...
        with torch.no_grad():
            ...
            v_total_loss, v_goal_loss, v_class_loss = combined_loss_fn(model, v_emb, v_goal, v_class_goal_onehot)
