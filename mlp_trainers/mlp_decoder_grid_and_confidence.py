...
from process_gemini_data import collect_train_test_data_grid, collect_train_test_data_from_embeddings_confidence
...
for llm, llm_name in llms.items():
    ...
    train, test = collect_train_test_data_from_embeddings_confidence(json_path=json_data_file,train_ratio=0.8,test_ratio=0.2, device="mps")
    ...
    for epoch in range(epochs):
        for i in range(train["task_embedding"].shape[0] // batch_size):
            ...
            goal /= confidence_goal.unsqueeze(1)
            ...
            with torch.no_grad():
                ...
                v_goal /= v_confidence.unsqueeze(1) 
